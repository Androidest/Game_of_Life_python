...
def add_cell(ol):
    ...
    for i in range(choice([1,2,3])):
        while 1:
            p=choice(nl)
            p1=(p[0]+choice(selection),p[1]+choice(selection))
            if p1 not in nl:
                nl.append(p1)
                p1 = (2*p0[0]-p1[0],p1[1]) #p[0].x对称
                if p1 not in nl:
                    nl.append(p1)
                break
    return nl
...
def move_cell(ol):
    ...
    while 1:
        p=choice(nl)
        p1=(p[0]+choice([-3,3]),p[1]+choice([-3,3]))
        if p1 not in nl:
            nl.remove(p)
            nl.append(p1)
            p = (2*p0[0]-p[0],p[1])
...
while 1:
    ...
    while flip < 7 :
        ...
        if len(deleted) == old_changes:
            flip+=1
        else: 
            old_changes = len(deleted)
            flip = 0
